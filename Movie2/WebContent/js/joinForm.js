...
function acnumCheck() {
	var exp = /^\d{6}-\d{7}$/;
	var acnumSave = document.getElementById("acnum");
	if (acnumSave.value.match(exp)) {
		document.getElementById("acnumCh").style.color = "blue";
		document.getElementById("acnumCh").innerHTML = "정상적";
	} else {
		document.getElementById("acnumCh").style.color = "red";
		document.getElementById("acnumCh").innerHTML = "비정상적";
	}
}
...
function checkSign() {
	...
	if (saveId.value.match(exp1)&& pass.value.match(exp2) && ps1 == ps2 &&phoneSave.value.match(exp3)
			&& address1 !=null&&acnumSave.value.match(exp4)) {
		if(idChecksave!=null){
