	...
	<table>
		...
		<c:forEach var="member" items="${select}">
			<tr>
				...
				<td><c:if test="${member.gchance eq 'NO'}">
						<button
							onclick="location.href='GoodPlus?column=GOODLIMIT&bnum=${member.bnum}'">좋아요${member.gcnum}</button>
					</c:if> <c:if test="${member.gchance eq 'YES'}">
						<button
							onclick="location.href='GoodMinus?column=GOODLIMIT&bnum=${member.bnum}'">좋아요${member.gcnum}</button>
					</c:if></td>
				<td><c:if test="${member.mchance eq 'NO'}">
						<button
							onclick="location.href='GoodPlus?column=MINUSLIMIT&bnum=${member.bnum}'">싫어요${member.gcnum}</button>
					</c:if> <c:if test="${member.mchance eq 'YES'}">
						<button
							onclick="location.href='GoodMinus?column=MINUSLIMIT&bnum=${member.bnum}'">싫어요${member.gcnum}</button>
					</c:if></td>
			</tr>
		</c:forEach>
	</table>
