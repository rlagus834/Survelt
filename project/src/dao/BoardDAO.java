...
public List<BoardDTO> updateSelectServiceAsc(){
	String sql="SELECT * FROM BOARDS ORDER BY COUNT DESC";
	List<BoardDTO> list=new ArrayList<BoardDTO>();
	try {
		pstmt=con.prepareStatement(sql);
		rs=pstmt.executeQuery();
...
public List<BoardDTO> writeOpen(String id,int boardnumber){
	String sql="SELECT * FROM BOARDS WHERE ID=? AND BOARDNUMBER=?";
	List<BoardDTO> list=new ArrayList<BoardDTO>();
	try {
		pstmt=con.prepareStatement(sql);
		pstmt.setString(1, id);
		...
		sql="UPDATE BOARDS SET COUNT=COUNT+1 WHERE ID=? AND BOARDNUMBER=?";
		pstmt=con.prepareStatement(sql);
		pstmt.setString(1, id);
		pstmt.setInt(2, boardnumber);
		pstmt.executeUpdate();
