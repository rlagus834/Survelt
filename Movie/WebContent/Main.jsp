...
<body>
	...
	[이전]&nbsp;
		...
		<c:if test="${paging.page>1}">
			<a
				href="boardListPaging?page=${paging.page-1}&filters=${requestScope.filters}&search=${requestScope.search}">[이전]</a>&nbsp;
	</c:if>
		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i"
			...
			${i}
				...
				<c:otherwise>
					<a
						href="boardListPaging?page=${i}&filters=${requestScope.filters}&search=${requestScope.search}">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
...
[다음]
		...
		<c:if test="${paging.page<paging.maxPage}">
			<a
				href="boardListPaging?page=${paging.page+1}&filters=${requestScope.filters}&search=${requestScope.search}">[다음]</a>
		</c:if>
		...
		<c:if test="${!empty sessionScope.id}">
			...
			<div>${sessionScope.id}님
				환영합니다. 프로필:<img src="fileUpload/${sessionScope.photo}"> <a
					href="PasswordCheck?id=${sessionScope.id}">정보수정</a>
